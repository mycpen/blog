    ...
    for (var i = start;i<endFor;i++){
      ...
      <div class="cf-article">
        ...
        <div class="cf-article-avatar no-lightbox flink-item-icon">
          <a onclick="openMeShow(event)" data-link="${item.link}" class="" target="_blank" rel="noopener nofollow" href="javascript:;"><img class="cf-img-avatar avatar" src="${item.avatar}" alt="avatar" onerror="this.src='${fcdata.error_img}'; this.onerror = null;"><span class="cf-article-author">${item.author}</span></a>
...
function changeEgg(){
  ...
  if(fcdata.jsonurl || fcdata.apiurl ){
    ...
    container.innerHTML = ""
    UrlNow = localStorage.getItem("urlNow")
    // UrlNow = 'https://hexo-circle.cpen.top/all?'
...
function FetchFriendCircle(sortNow,changeUrl){
  ...
  // var fetchUrl = 'https://hexo-circle.cpen.top/all?' + "rule="+sortNow+"&start=0&end="+end
  if(changeUrl){
